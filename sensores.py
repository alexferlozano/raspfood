...
class Sensores:
    def __init__(self, echo, trigger, echo2, trigger2):
        ...
        self.e2 = echo2
        self.tr = trigger2
        self.token = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1aWQiOjMsImlhdCI6MTY3MDQ1ODY4Mn0.lckN0mYGNJmY6rcAvHtXOgSaToRcw9j5fMidYZEn77o'
        self.dhtDevice = adafruit_dht.DHT11(board.D18)
        GPIO.setmode(GPIO.BCM)
        GPIO.setup(self.e2, GPIO.IN)
        GPIO.setup(self.tr, GPIO.OUT)
    ...
    def getUltraData(self):
        try:
            ...
            GPIO.output(self.tr, 1)
            time.sleep(0.00001)
            GPIO.output(self.tr, 0)
            
            start_time = time.time()
            end_time = time.time()
            
            while GPIO.input(self.e2) == 0:
                start_time = time.time()
            
            while GPIO.input(self.e2) == 1:
                end_time = time.time()
